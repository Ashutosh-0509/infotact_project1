...
export default function Dashboard() {
  ...
  useEffect(() => {
    ...
    const demoOrders = [
      ...
      { id: '2', orderNumber: '#T89342', customerName: 'Priya Sharma', total: 850, status: 'completed', createdAt: new Date().toISOString(), paymentMethod: 'Cash', items: 2 },
      ...
      { id: '4', orderNumber: '#T89344', customerName: 'Sunita Devi', total: 2100, status: 'completed', createdAt: new Date().toISOString(), paymentMethod: 'UPI', items: 6 },
  ...
  const handleExportExcel = async () => {
    ...
    try {
      const data = recentOrders.map(order => ({
        ...
        'Date': formatDate(new Date(order.createdAt as unknown as string))
      }));
      ...
      xlsx.writeFile(wb, `Recent_Transactions_${new Date().toISOString().split('T')[0]}.xlsx`);
  ...
  const handleExportPDF = async () => {
    ...
    try {
      const doc = new jsPDF();
      doc.setFontSize(16);
      doc.text("Retail Pro - Recent Transactions", 14, 22);
      doc.setFontSize(10);
      ...
      const tableRows = recentOrders.map(order => [
        order.orderNumber,
        order.items?.toString() || '0',
        formatCurrency(order.total),
        order.paymentMethod,
        formatDate(new Date(order.createdAt as unknown as string))
      ]);
      ...
      doc.save(`Recent_Transactions_${new Date().toISOString().split('T')[0]}.pdf`);
      toast.success('Report downloaded successfully!', { id: toastId });
    } catch (e) {
            ...
            <div className="flex gap-3">
               ...
               <DropdownMenu>
                  ...
                  <DropdownMenuContent align="end" className="bg-card/90 backdrop-blur-xl border-primary/20 rounded-xl overflow-hidden shadow-2xl">
                    ...
                    <DropdownMenuItem onClick={handleExportPDF} className="px-4 py-2 cursor-pointer font-bold text-[10px] uppercase tracking-widest focus:bg-primary/10 focus:text-primary">EXPORT AS PDF</DropdownMenuItem>
                    <DropdownMenuItem onClick={handleExportExcel} className="px-4 py-2 cursor-pointer font-bold text-[10px] uppercase tracking-widest focus:bg-primary/10 focus:text-primary">EXPORT AS EXCEL</DropdownMenuItem>
              ...
              <div className="space-y-6">
                <div className="grid grid-cols-2 gap-4">
                   <div className="p-4 rounded-3xl bg-black/30 border border-white/5">
                      ...
                      <p className="font-bold text-sm uppercase">{formatDate(new Date(selectedTransaction.createdAt as unknown as string), 'long')}</p>
                   </div>
                </div>
