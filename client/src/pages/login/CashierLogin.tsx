...
import { ShoppingCart, EyeOff, Eye, ArrowLeft, Loader2, AlertCircle } from 'lucide-react';
...
export default function CashierLogin() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const result = await login({ email, password, role: 'Cashier' });
    
    if (result.success) {
      toast.success('Logged in as Cashier');
    } else {
  ...
  return (
    <div className="min-h-screen w-full flex bg-background">
      ...
      <div className="hidden lg:flex w-1/2 flex-col justify-between p-12 lg:p-24 relative overflow-hidden" 
        ...
        <div className="relative z-10">
          ...
          <motion.p 
            ...
            className="text-blue-100 text-lg lg:text-xl max-w-md leading-relaxed"
          >
            Process sales, manage cart, and handle fast customer billing
          </motion.p>
        </div>
      ...
      <div className="w-full lg:w-1/2 flex flex-col justify-center p-8 lg:p-24 relative bg-white">
        ...
        >
          <div className="mb-8">
            <h2 className="text-3xl font-bold" style={{ color: '#2563eb' }}>Cashier Login</h2>
            <p className="text-muted-foreground mt-2">Enter your credentials to access POS</p>
          </div>
          ...
          <form onSubmit={handleSubmit} className="space-y-5">
            ...
            <Button
              type="submit"
              disabled={loading}
              className="w-full h-11 text-base font-medium rounded-full hover:opacity-90 transition-opacity"
          ...
          <div className="mt-8 pt-8 border-t border-border">
            ...
            <button
              onClick={fillDemo}
              className="w-full p-4 rounded-xl border border-blue-100 bg-blue-50/50 hover:bg-blue-50 transition-colors text-left flex justify-between items-center group"
            >
              <div>
                <p className="text-sm font-semibold text-blue-900">Cashier</p>
                <p className="text-xs text-blue-600/80 mt-1">cashier@pos.com / cashier123</p>
              </div>
