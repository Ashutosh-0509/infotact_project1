...
export default function POS() {
  ...
  const categories = useMemo(() => {
    const cats = new Set(products.map((p: Product) => p.categoryId));
                ...
                {categories.map((catId: any) => {
                  return (
                    <TabsTrigger key={catId} value={catId} className="px-6">
                      {catId?.toString().includes('-') ? `Category ${catId.split('-')[1]}` : catId}
                    </TabsTrigger>
                  );
                })}
              </TabsList>
        ...
        <DialogContent className="max-w-md">
          ...
          <DialogFooter>
            ...
            <Button
              ...
              disabled={
                (paymentMethod === 'cash' && (!amountReceived || parseFloat(amountReceived) < cartTotal.total)) ||
                orderMutation.isPending
              }
              className="gap-2"
            >
              <Printer className="h-4 w-4" />
              {orderMutation.isPending ? 'Processing...' : 'Complete & Print'}
            </Button>
          </DialogFooter>
        </DialogContent>
      </Dialog>
