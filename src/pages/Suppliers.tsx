...
import { Supplier, Product, formatCurrency, formatDate } from '@/lib/index';
...
export default function Suppliers() {
  ...
  // Since we also want to visually reflect add/edit/delete,
  // we combine server data + local modifications here if needed,
  // but for simplicity let's stick to reading from 'suppliers' directly 
  // and performing local mod in memory just for the demo:
  const displaySuppliers = localSuppliers.length > 0 ? localSuppliers : suppliers;

  const filteredSuppliers = displaySuppliers.filter((supplier: any) =>
    ...
    },
    {
      ...
      render: (supplier: Supplier) => (
        <div className="flex items-center gap-2">
          ...
          <span className="text-foreground font-medium">{supplier.productsSupplied ? supplier.productsSupplied.length : 0}</span>
    ...
    },
    {
      ...
      onClick: (supplier: Supplier) => {
        if (confirm(`Are you sure you want to delete ${supplier.name}?`)) {
          setLocalSuppliers(displaySuppliers.filter(s => s.id !== supplier.id));
        }
      },
      variant: 'destructive',
    },
  ...
  const handleSubmit = (data: Partial<Supplier>) => {
    if (selectedSupplier) {
      setLocalSuppliers(displaySuppliers.map(s =>
        s.id === selectedSupplier.id ? { ...s, ...data, updatedAt: new Date() } as Supplier : s
      ));
    } else {
      ...
      setLocalSuppliers([...displaySuppliers, newSupplier]);
    }
    setIsFormOpen(false);
    setSelectedSupplier(undefined);
  };
  ...
  const getSuppliedProducts = (productIds: string[]) => {
    ...
    return products.filter((p: Product) => productIds.includes(p.id));
  };

  const activeSuppliers = displaySuppliers.filter(s => s.isActive !== false).length;
  const totalPurchaseValue = displaySuppliers.reduce((sum, s) => sum + (s.totalPurchases || 0), 0);
      ...
      <div className="space-y-6">
        ...
        >
          <Card>
            ...
            <CardContent>
              <div className="text-2xl font-bold">{displaySuppliers.length}</div>
          ...
          <Card>
            ...
            <CardContent>
              <div className="text-2xl font-bold">
                {displaySuppliers.reduce((sum, s) => sum + (s.productsSupplied?.length || 0), 0)}
              </div>
              <p className="text-xs text-muted-foreground mt-1">
                ...
                <TabsContent value="products" className="mt-6">
                  <Card>
                    <CardHeader>
                      ...
                      <CardDescription>
                        {viewSupplier.productsSupplied ? viewSupplier.productsSupplied.length : 0} products from this supplier
                      </CardDescription>
                    </CardHeader>
                    <CardContent>
                      <div className="space-y-3">
                        ...
                        {(!viewSupplier.productsSupplied || viewSupplier.productsSupplied.length === 0) && (
